...
def main():
    ...
    issues=validate_rows(source_index(),lineage_index(),deep_reading_index(),rows)
    if issues:fail(f"issues={len(issues)}; "+"; ".join(f"{a}: {b}" for a,b in issues[:20]))
    print("k2-enumeration-compression: PASS")
    print(f"rows={len(rows)} collapsed_entries={sum(r.get('enumerated_entries_count',0) for r in rows)} issues=0")
