...
CHECK_KEYS={"evidence_state_complete","unknown_textual_backlog_cleared","evidence_state_unblocks_claim_extraction","project_k2_not_blocked","project_evidence_extraction_open","required_domains_consistent","qcic_materialization_current"}
OBS_KEYS={"project_phase","project_next_phase","evidence_status","unknown_textual_resolution_backlog","qcic_stance_topic_count","qcic_enumeration_unit_count","qcic_claim_eligible_unit_count"}
...
def shape_issues(v):
    ...
    if isinstance(obs,dict):
        for k in ("unknown_textual_resolution_backlog","qcic_stance_topic_count","qcic_enumeration_unit_count","qcic_claim_eligible_unit_count"):
            x=obs.get(k)
    ...
    if not isinstance(gf,list) or len(gf)<3 or len(gf)!=len(set(gf)):issues.append("generated_from must be unique non-empty list")
    return issues
