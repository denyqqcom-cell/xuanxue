...
def fixture_input():
    return {
        ...
        "statistical_preregistration_sha256": "2" * 64,
    }
...
def main():
    ...
    validate_registration_input(copy.deepcopy(source), copy.deepcopy(contract), fixture_only=True)
    first = bind_batch_registration_candidate(copy.deepcopy(source), copy.deepcopy(contract), fixture_only=True)
    second = bind_batch_registration_candidate(copy.deepcopy(source), copy.deepcopy(contract), fixture_only=True)
    ...
    x = fixture_input()
    x["outcome"] = "LEAK"
    must_fail(validate_registration_input, x, copy.deepcopy(contract), True)
    negative_cases += 1
    ...
    x = fixture_input()
    x["statistical_preregistration_sha256"] = "bad"
    must_fail(validate_registration_input, x, copy.deepcopy(contract), True)
    negative_cases += 1
    ...
    assert negative_cases == 12
    print(
        "k2-qimen-p2-batch-registration-tests: PASS "
        "negative_cases=12 fixture_only=true production_batch_created=false registry_persisted=false "
