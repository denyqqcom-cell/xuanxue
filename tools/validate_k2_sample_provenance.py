...
def policy_index(rows):
    issues=[];out={}
    if not rows:
        return out,[("<sample-provenance-policy>","sample provenance policy registry must contain at least one version")]
    for row in rows:
        version=row.get("policy_version") or "<missing>"
        if set(row)!=POLICY_FIELDS:
            issues.append((version,f"sample provenance policy fields mismatch missing={sorted(POLICY_FIELDS-set(row))} extra={sorted(set(row)-POLICY_FIELDS)}"))
        ...
        for field in [
            "require_preoutcome_binding","require_fingerprint_on_every_freeze","require_unique_within_batch",
            "require_unique_within_replication_cohort","raw_identity_repository_storage_forbidden",
            "secret_repository_storage_forbidden","research_only",
        ]:
...
def validate_records(batches,freezes,bindings,policies):
    issues=[]
    policy_by_version,policy_issues=policy_index(policies)
    issues.extend(policy_issues)
    batch_by_id={b.get("batch_id"):b for b in batches}
    freezes_by_batch={}
    for f in freezes:
        freezes_by_batch.setdefault(f.get("batch_id"),[]).append(f)

    binding_by_batch={};seen_binding_ids=set()
    for row in bindings:
        rid=row.get("binding_id") or "<missing>";bid=row.get("batch_id")
        if set(row)!=BINDING_FIELDS:
            issues.append((rid,f"sample provenance binding fields mismatch missing={sorted(BINDING_FIELDS-set(row))} extra={sorted(set(row)-BINDING_FIELDS)}"))
        ...
        if not batch:
            issues.append((rid,f"sample provenance binding references unknown batch_id: {bid}"))
            continue
        ...
        freeze_times=[pv.utc_value(f.get("frozen_at_utc")) for f in freezes_by_batch.get(bid,[])]
        freeze_times=[x for x in freeze_times if x]
    ...
    for f in freezes:
        fid=f.get("freeze_id") or "<missing>";bid=f.get("batch_id")
        binding=binding_by_batch.get(bid)
        ...
        if not isinstance(payload,dict):
            issues.append((fid,"sample provenance requires frozen_payload object"))
            continue
        ...
        if policy is not None:
            ...
            if version!=policy.get("policy_version"):issues.append((fid,"freeze sample_provenance_policy_version must match preregistered binding"))
            if policy_sha!=pv.canonical_sha256(policy):issues.append((fid,"freeze sample_provenance_policy_sha256 must match preregistered binding"))
            if key_id!=policy.get("fingerprint_key_id"):issues.append((fid,"freeze sample_fingerprint_key_id must match registered policy key id"))
...
def main():
    ...
    issues=validate_records(batches,freezes,bindings,policies)
    if issues:fail(f"issues={len(issues)} first={issues[0][0]}: {issues[0][1]}")
    print("k2-sample-provenance: PASS")
    print(f"policies={len(policies)} bindings={len(bindings)} batches={len(batches)} freezes={len(freezes)} issues=0")
