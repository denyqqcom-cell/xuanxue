#!/usr/bin/env python3
"""Audit QM-SRC-0017 截路空亡 stated rule against its own printed examples.

This stays strictly source-internal: pdf:p283 is compared with the printed
examples at pdf:p294-p306. Mismatches are retained as unresolved source tension;
no external口诀 or canonical tradition is used to repair the carrier.
"""

BRANCHES=set("子丑寅卯辰巳午未申酉戌亥")
STEMS=set("甲乙丙丁戊己庚辛壬癸")
FOUR_BRANCH_STEMS={"戊","癸"}
...
def _validate_fixture(data):
    ...
    for stem,branches in rules.items():
        expected_len=4 if stem in FOUR_BRANCH_STEMS else 2
        if not isinstance(branches,list) or len(branches)!=expected_len or len(set(branches))!=expected_len or any(b not in BRANCHES for b in branches):
            raise ValueError(f"invalid stated 截路空亡 branches for {stem}")
    ...
    for row in examples:
        ...
        observed=row.get("observed_jielukongwang_hour_branches")
        if not isinstance(observed,list) or len(observed)!=len(set(observed)) or any(b not in BRANCHES for b in observed):
            raise ValueError(f"invalid observed 截路空亡 branches for {cid}")
...
def audit_fixture(data):
    ...
    for row in data["examples"]:
        ...
        expected=rules[stem]
        observed=row["observed_jielukongwang_hour_branches"]
