...
ALLOWED={
  "distillate_id","source_id","work_id","domain","distillation_scope","reading_ref",
  "canonical_sha256","source_anchors","course_family_id","course_role","independence_policy",
  "prior_distillate_refs","evidence_reaudit_coverage","source_credit","empirical_credit",
  "essence","method_map","applicability_constraints","source_limitations","conflicts_and_tensions",
  "anti_patterns","model_updates","testable_hypotheses","excluded_from_operational_use",
  "claim_extraction_blocked","acceptance_status","distillation_status","review_status","copyright_class"
}
...
def indexes(root=ROOT):
    ...
    lineage={r.get("source_id"):r for r in load_jsonl(k/"K2_SOURCE_LINEAGE.jsonl")}
    ...
    prior=prior_distillate_index(root)
    reaudit={}
    state_path=k/"K2_EVIDENCE_REAUDIT_STATE.json"
    if state_path.exists():
        state=load_json(state_path)
        reaudit={t.get("source_id"):t.get("coverage") for t in state.get("targets") or []}
...
def inspect(row,idx):
    ...
    if not lin or row.get("work_id")!=lin.get("work_id"):issues.append((did,"work_id mismatch with source lineage"))
    ...
    if not reading:issues.append((did,"unknown reading_ref"))
        ...
        if isinstance(pages,int):
            if reading.get("page_start")!=1 or reading.get("page_end")!=pages or reading.get("pages_reviewed_count")!=pages:
                issues.append((did,"reading_ref does not cover complete canonical source"))
    ...
    if course:
        ...
        if course.get("independent_vote_allowed") is False and row.get("independence_policy")!="COURSE_FAMILY_SINGLE_VOTE":issues.append((did,"same-course source must use COURSE_FAMILY_SINGLE_VOTE"))
    else:
        ...
        if row.get("independence_policy")!="DEFAULT":issues.append((did,"non-course source must use DEFAULT independence policy"))
    ...
    expected_reaudit=reaudit.get(sid)
    actual_reaudit=row.get("evidence_reaudit_coverage")
    if expected_reaudit is None:
        if actual_reaudit!="NOT_APPLICABLE":issues.append((did,"source without reaudit target must use NOT_APPLICABLE"))
    elif actual_reaudit!=expected_reaudit:
        issues.append((did,f"evidence_reaudit_coverage {actual_reaudit} != current target coverage {expected_reaudit}"))

    if row.get("distillation_scope")!="DEEP_SOURCE_BOOK":issues.append((did,"distillation_scope must be DEEP_SOURCE_BOOK"))
...
def validate_rows(rows,state,idx):
    ...
    for row in rows:
        ...
        seen_d.add(did);seen_s.add(sid)
        issues.extend(inspect(row,idx))
    ...
    if state.get("status")=="COMPLETE" and any(t.get("required") is True and t.get("source_id") not in seen_s for t in targets):
        issues.append(("<state>","status COMPLETE with missing required target"))
    return issues


def main():
    project=load_json(K/"PROJECT_STATE.json")
    state=load_json(K/"K2_DEEP_SOURCE_DISTILLATION_STATE.json")
    ...
    rows=load_jsonl(K/"K2_DEEP_SOURCE_DISTILLATES.jsonl")
    issues=validate_rows(rows,state,indexes(ROOT))
