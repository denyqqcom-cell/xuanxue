...
FREEZE_FIELDS={
    "freeze_id","plan_id","case_id","frozen_at_utc","model_commit_sha",
...
def string_list(v):
    return isinstance(v,list) and bool(v) and all(nonempty_text(x) for x in v)
...
def validate_records(distillates,plans,freezes,outcomes):
    ...
    for p in plans:
        ...
        for f in ["model_name","comparator_name","question_scope","unit_of_analysis","success_condition","failure_condition","abstention_rule","high_risk_policy","update_policy"]:
            if not nonempty_text(p.get(f)):issues.append((pid,f"{f} must be non-empty text"))
    ...
    freeze_by_id={}
    for f in freezes:
        fid=f.get("freeze_id") or "<missing>";pid=f.get("plan_id")
        ...
        ts=f.get("frozen_at_utc")
        if not isinstance(ts,str) or not UTC_RE.match(ts):issues.append((fid,"frozen_at_utc must be UTC second timestamp ending Z"))
    ...
    for o in outcomes:
        ...
        if not fr:issues.append((oid,f"unknown freeze_id: {fid}"))
            ...
            fts=fr.get("frozen_at_utc");ots=o.get("observed_at_utc")
            if isinstance(fts,str) and UTC_RE.match(fts) and isinstance(ots,str) and UTC_RE.match(ots):
                fd=datetime.strptime(fts,"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
                od=datetime.strptime(ots,"%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
                if od<=fd:issues.append((oid,"outcome must be observed after freeze"))
        if not isinstance(o.get("observed_at_utc"),str) or not UTC_RE.match(o.get("observed_at_utc","")):issues.append((oid,"observed_at_utc must be UTC second timestamp ending Z"))
...
def main():
    ...
    issues=validate_records(distillates,plans,freezes,outcomes)
    if issues:
