...
def load_rows(path=REG):
    ...
    for n,raw in enumerate(path.read_text(encoding="utf-8").splitlines(),1):
        ...
        try:r=json.loads(raw)
            ...
            raise ValueError(f"invalid JSONL {path}:{n}: {e}")
...
def validate_rows(rows):
    ...
    for i,r in enumerate(rows,1):
        ...
        if st=="RESOLVED":
            ...
            issues.append((cid,"non-RESOLVED row must have outcome_class=null"))
...
def main():
    ...
    if issues:
        cid,msg=issues[0]
        print(f"k2-prospective-cases: FAIL: issues={len(issues)} first={cid}: {msg}",file=sys.stderr);raise SystemExit(1)
