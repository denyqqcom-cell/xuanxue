...
STATUS="PARTIAL_READING_CONFIRMED"
LAYERS={
...
def validate_rows(sources,holds,evidence_rows):
    ...
    for h in holds:
        ...
        if sid not in sources:issues.append((sid,"unknown source_id"))
        if h.get("status")!=STATUS:issues.append((sid,"status must be PARTIAL_READING_CONFIRMED"))
...
def main():
    ...
    issues=validate_rows(source_ids(ROOT),holds,evidence)
    if issues:
