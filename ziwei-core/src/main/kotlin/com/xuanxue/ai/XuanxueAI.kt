...
object XuanxueAI {
    ...
    val tools: List<Map<String, Any>> = listOf(
        ...
        mapOf("name" to "qimen_interpret", "description" to "奇门遁甲盘解读：局、值符值使、八门吉凶"),
    ...
    fun qimen(c: QimenEngine.QimenChart): Reading =
        Reading("qimen", QimenInterpreter.interpret(c).map { ReadingItem("奇门", it) })
